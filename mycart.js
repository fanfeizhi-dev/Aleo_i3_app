...
function checkWalletConnection() {
    ...
    return {
        ...
        tokens: userInfo.credits, // 使用 ALEO 余额
        ...
        row.innerHTML = `
            ...
            <td class="price-display">
                <div class="purchase-option">
                    <div class="price-info">${tokenPricePerCall.toFixed(6)} ALEO/call</div>
                    ...
                    <div class="subtotal-small">Subtotal: ${tokenSubtotal} ALEO (${tokenQuantity.toLocaleString()} API calls)</div>
                </div>
            </td>
            <td class="price-display">
                <div class="purchase-option">
                    <div class="price-info">${sharePriceUsdc.toFixed(6)} ALEO</div>
                    <div class="quantity-controls">
                    ...
                    <div class="subtotal-small">Subtotal: ${shareSubtotal} ALEO (${shareQuantity} shares)</div>
                </div>
            </td>
            <td class="total-subtotal">
                <div class="total-amount">${(parseFloat(tokenSubtotal) + parseFloat(shareSubtotal)).toFixed(6)} ALEO</div>
            </td>
            <td>
...
function showCheckoutModal() {
                ...
                orderItemsHtml += `
                    <div class="order-item">
                        ...
                        <div class="order-item-details">
                            ${tokenQuantity > 0 ? `${tokenQuantity.toLocaleString()} API calls (${tokenSubtotal.toFixed(6)} ALEO)` : ''}
                            ${tokenQuantity > 0 && shareQuantity > 0 ? ' + ' : ''}
                            ${shareQuantity > 0 ? `${shareQuantity} shares (${shareSubtotal.toFixed(6)} ALEO)` : ''}
                        </div>
                    </div>
                `;
            }
    ...
    document.getElementById('modalTotal').textContent = `${grandTotal.toFixed(6)} ALEO`;
...
function closeCheckoutModal() {
    ...
    const balanceInfo = modalBody.querySelector('div[style*="Your ALEO Balance"]');
    if (balanceInfo) {
        balanceInfo.remove();
    }
}
...
function placeOrder() {
    ...
    (async () => {
        ...
        message += '\n📋 Click the notification at bottom-right to view transaction details.';
...
function showPurchaseSuccessToast(signature, order, explorerUrl) {
    try {
        ...
        const amount = order.amount ? `${order.amount.toFixed(6)} ALEO` : 'N/A';
